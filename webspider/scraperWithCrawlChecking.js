...
//Used to communicate with mongodb (Source tutorial: http://www.atinux.fr/2011/10/15/tutoriel-sur-mongoose-mongodb-avec-node-js/)
...
function extract_links(page_url, html_str){
  ...
  (html_str.match(EXTRACT_URL_REG) || []).forEach(function(url){
    ...
    var query = newUrlModel.find(null);
    query.where('url',url);
    query.exec(function(err,myUrls){
      if(err) { throw err; }
      if(myUrls.length!=0){
        console.log('This URL has already been crawled');
        ...
        myNewUrl.save(function (err) {
          if (err) { throw err; }
          console.log('URL has been added to our DB with success');
          mongoose.connection.close();
        });
      }
    });  
